...
export async function prepareContracts(thisObject: Mocha.Context, signer: SignerWithAddress) {
    const contractFactory = await ethers.getContractFactory("BookmarkNFT");
    const contract1 = await contractFactory.connect(signer)
        .deploy("name", "j5s", "description", ethers.utils.parseUnits("0.01"));
    thisObject.contract1 = contract1;
}
